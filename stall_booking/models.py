...
class SubSession(models.Model):
    thematic_session = models.ForeignKey(
        ThematicSession, on_delete=models.CASCADE, related_name="sub_sessions"
    )
    title = models.CharField(max_length=255)
    description = models.TextField(null=True, blank=True)
...
class Panelist(models.Model):
    ...
    thematic_session = models.ManyToManyField(
        ThematicSession, related_name="thematicpanelists", blank=True
    )
    sub_session = models.ManyToManyField(
        SubSession, related_name="panelists", blank=True
    )
...
class ThematicRegistration(models.Model):
    ...
    hotel_accomodation = models.CharField(
        max_length=20, choices=HOTEL_ACCOMODATION_CHOICES, null=True, blank=True
    )
...
class GuidedTour(models.Model):
    ...
    student_level = models.CharField(
        max_length=20, choices=STUDENT_LEVEL_CHOICES, default="10+2"
    )
